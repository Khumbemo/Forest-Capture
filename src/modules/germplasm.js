...
const BODIES = {
  icfre: { id:"icfre", name:"ICFRE", full:"Indian Council of Forestry Research & Education", ministry:"MoEF&CC, Govt. of India", focus:"Field Collection & Seed Certification", icon:"🌳", tag:"Forest Seed Management Manual · SOPs 2023", desc:"For forest seed field collection, seed zone demarcation, reproductive material categorisation, and stand/mother-tree documentation as per ICFRE/MoEF guidelines." },
  nbpgr: { id:"nbpgr", name:"ICAR-NBPGR", full:"National Bureau of Plant Genetic Resources", ministry:"ICAR, Govt. of India", focus:"Ex-Situ Genebank Conservation", icon:"🏛️", tag:"National Genebank Standards · Passport Data Protocol", desc:"For accession passport documentation, IC number assignment, drying/storage protocols, seed viability, plant quarantine clearance, and MTA compliance for the National Genebank." },
  ista: { id:"ista", name:"ISTA / OECD", full:"International Seed Testing Association / OECD Forest Scheme", ministry:"International Standards (adopted by India)", focus:"Seed Testing & Trade Certification", icon:"🔬", tag:"ISTA Rules 2024 · OECD Forest Seed Scheme", desc:"For seed lot certification, purity analysis, germination testing, moisture content, 1000-seed weight, vigour testing, and OECD forest scheme certification for domestic/international trade." }
};
...
function getSelectHtml(id, label, options, placeholder="Select...", full=false, tip="") {
  ...
  return `
    <div class="form-group" ${full ? 'style="flex: 0 0 100%;"' : ''}>
      <label for="${id}">${label}</label>
      <select id="${id}"><option value="">${placeholder}</option>${opts}</select>
      ${tip ? `<div style="font-size:0.75rem; color:var(--text-muted); margin-top:2px;">${tip}</div>` : ''}
    </div>
  `;
}
...
function getSection(title, icon, children) {
  return `
    <div class="form-card" style="margin-bottom:var(--sp-md);">
...
function renderISTA() {
  ...
  h += getSection("Purity Analysis (ISTA Sec 10)", "🔍",
  ...
  h += getSection("Germination Test (ISTA Sec 5)", "🌱",
...
function getHeaderHtml(title) {
  return `
    <div class="screen-header">
      <h2 style="margin-left: 0;">${title}</h2>
    </div>
...
export async function refreshGermplasmUI() {
  ...
  if (currentView === 'form' && currentBody) {
    ...
    mount.innerHTML = getHeaderHtml(`New Record &nbsp;<span style="font-weight:400;font-size:0.8em;opacity:0.7;">${b.icon} ${b.name}</span>`) + `
      <div style="padding:var(--sp-md);">
        ${fieldsHtml}
        <button id="btnGSaveRec" class="btn btn-primary btn-block" style="margin-top:var(--sp-md);">Save Record</button>
        <button id="btnGCancelRec" class="btn btn-ghost btn-block" style="margin-top:var(--sp-sm); border:1px solid var(--border);">← Back to Forms</button>
      </div>
    `;
  ...
  if (currentView === 'list') {
    ...
    const applyFilter = (fBody) => {
      ...
      if (filtered.length === 0) {
        cont.innerHTML = `
          <div class="empty-state" style="padding:var(--sp-xl) 0;text-align:center;color:var(--text-muted);">
            <div style="font-size:2.5rem;margin-bottom:var(--sp-sm);">🌱</div>
        ...
        cont.innerHTML = filtered.map(r => {
          ...
          return `
            <div class="form-card" style="margin-bottom:var(--sp-md); position:relative;">
              <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                  <div style="font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase; letter-spacing:0.5px;">${body.icon} ${body.name}</div>
                  <div style="font-size:1.1rem; color:var(--text); font-weight:700; font-style:italic; margin-top:4px;">${r.speciesScientific || '—'}</div>
              ...
              <div style="font-size:0.8rem; color:var(--text-muted); margin-top:12px; display:flex; gap:12px; flex-wrap:wrap;">
...
function renderHome() {
  ...
  Store.getActive().then(sv => {
    ...
    let bHtml = Object.keys(BODIES).map(k => {
      const b = BODIES[k];
      return `
        <div class="form-card g-body-card" data-b="${k}" style="margin-bottom:var(--sp-md); cursor:pointer; transition:transform 0.2s;">
          <div style="display:flex; align-items:flex-start; gap:var(--sp-md);">
            <div style="font-size:2.5rem; line-height:1;">${b.icon}</div>
            <div style="flex:1;">
              <h3 class="card-title" style="margin-bottom:4px;">${b.name} <span class="badge" style="background:var(--bg-input); font-size:0.65rem;">${b.focus}</span></h3>
              <div style="font-size:0.75rem; font-weight:700; color:var(--text-muted); margin-bottom:8px;">${b.full}</div>
